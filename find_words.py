...
def find_matching_movies(df, search_words):
    ...
    for index, row in df.iterrows():
        movie_title = row['Title'] if 'Title' in row else row[0]  # Handle different column names
        match_count = 0
        words_matched = set()
        ...
        # Store the match count and matched words if there were any matches
        if match_count > 0:
    ...
    for title, count in sorted_movies[:3]:
        top_matches.append((title, count, matched_words[title]))
